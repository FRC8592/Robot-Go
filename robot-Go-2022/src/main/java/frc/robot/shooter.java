...
/**move turret to drive x to be less than "TURRET_ERROR"*/
  ...
  public void teleopmoveTurret(XboxController ballshootController){
    if (ballshootController.getLeftBumper()){
      this.moveTurret();
    }
    else{
      double turretRotation = ballshootController.getLeftX() * MANUAL_POWER; //manual turret movement
  ...
  public void teleopBall(XboxController ballshootController){
    ...
    else if (ballshootController.getRightTriggerAxis() == 1){
      this.shootBall();
    }
    else {
      this.stopBall();
